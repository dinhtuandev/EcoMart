...
const getStatusConfig = (status?: OrderStatus): { label: string; color: string } => {
  if (!status) return { label: '—', color: 'bg-slate-100 text-slate-700' };
...
// Main Component
// =============================================
export const ManagerOrderPage: React.FC = () => {
                      ...
                      <tr key={order.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-1.5 flex-nowrap">
                            ...
                            <Link
                              to={isManagerRoute ? `/manager/orders/${order.id}` : `/admin/orders/${order.id}`}
